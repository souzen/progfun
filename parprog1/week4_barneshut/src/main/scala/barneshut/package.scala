...
package object barneshut {
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body]) extends Quad {
    ...
    val mass = bodies.foldLeft(0.0f)((acc, b: Body) => acc + b.mass)
    val massX = bodies.foldLeft(0.0f)((acc, b: Body) => acc + b.mass * b.x) / mass
    val massY = bodies.foldLeft(0.0f)((acc, b: Body) => acc + b.mass * b.y) / mass
    ...
    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        ...
        (bodies :+ b).foldLeft(fork)((acc: Fork, body: Body) => acc.insert(body))
      }
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      ...
      val sectorX = clamp(b.x, boundaries.minX, boundaries.maxX)
      val sectorY = clamp(b.y, boundaries.minY, boundaries.maxX)

      val secX = normalize(sectorX, boundaries.minX, boundaries.maxX) * sectorPrecision
      val secY = normalize(sectorY, boundaries.minY, boundaries.maxY) * sectorPrecision

      val x = clamp(secX, 0, sectorPrecision - 1).toInt
      val y = clamp(secY, 0, sectorPrecision - 1).toInt
