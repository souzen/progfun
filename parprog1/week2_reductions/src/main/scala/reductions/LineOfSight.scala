...
object LineOfSight {
  ...
  def angle(xi: Float, i: Int): Float = {
    if (i == 0) 0.0f else xi / i
  }
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    while (i < input.length) {
      maxAngle = max(maxAngle, angle(input(i), i))
      output(i) = maxAngle
      i = i + 1
    }
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    output(from) = max(startingAngle, angle(input(from), from))
    for (i <- from + 1 until until) {
      output(i) = max(output(i - 1), angle(input(i), i))
    }
  }
