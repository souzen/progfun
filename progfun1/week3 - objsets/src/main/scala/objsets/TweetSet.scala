...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    val max = if (!left.isEmpty) {
      ...
      if (leftMax.retweets < elem.retweets) elem else leftMax
    }
    else elem

    if (!right.isEmpty) {
      val rightMax = right.mostRetweeted
      if (rightMax.retweets < max.retweets) max else rightMax
    } else max
  }
...
object GoogleVsApple {
  ...
  lazy val g: TweetList = googleTweets.descendingByRetweet
  lazy val a: TweetList = appleTweets.descendingByRetweet
}
